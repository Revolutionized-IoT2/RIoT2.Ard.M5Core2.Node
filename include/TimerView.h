...
// state machine, minus the encoder): Setting (lv_roller minute picker +
// Start button) -> Running (read-only lv_arc counting down, ticked by an
// lv_timer every second, + Cancel button) -> Done ("Time's up!" + Dismiss
// button, optionally beeping a non-blocking "egg ring" pattern). Publishes
// Report{id, "0"} exactly once on completion.
//
